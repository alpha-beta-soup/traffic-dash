...
class Camera extends React.Component {
  ...
  componentDidMount() {
    ...
    if (!this.loadImageInterval) {
      ...
      const timeoutFunc = function () { this.handleImageTimeout(); }.bind(this);
    ...
    if (!this.loadTrafficInterval) {
      ...
      const timeoutFunc = function () { this.handleTrafficTimeout(); }.bind(this);
  ...
  getImageData() {
    ...
    axios.get(
      ...
      const saveFunc = USE_LOCAL_CACHE ? saveJpegBufferLocally : uploadJpegBufferToS3;
  ...
  handlePinClick(event) {
    if (event) {
      console.log('pinClick', { event, id: this.props.id });
  ...
  handleAnimateClick(event) {
    ...
    if (event) {
      ...
      .then((images) => {
        const frame = this.state.animationFrame;
        if (!images.length || !images[frame]) {
          this.setState({ isAnimating: false });
          return;
        }
        const fileName = images[frame].split('/')[1];
        const src = makeImgSrc(this.props.id, fileName);
        this.setState({
          isAnimating: true,
        ...
        this.advanceAnimationFrame = setInterval(() => {
          ...
          if (!nextImage) return;
          const fileName = nextImage.split('/')[1];
          const src = makeImgSrc(this.props.id, fileName);
          this.setState({
            src,
  ...
  render() {
    ...
    return (
      <Card key={this.props.id} className={this.props.card}>
        ...
        />
        <CameraTime
          time={cameraReportedTime}
        />
        <CameraContent
          name={this.props.name}
          weather={this.state.weather}
          description={replaceAll(this.props.description, descriptionReplace)}
        />
        <CameraActions
          share={this.handleShareClick}
          focus={this.handleFocusClick}
          animate={this.handleAnimateClick}
          pin={this.handlePinClick}
