...
class AppGrid extends React.Component {
  ...
  getVMS() {
    ...
    instance.post().then(res => {
      xml2js(res.data, (err, data) => {
        const parsedSigns = _.filter(parseSigns(data['tns:getSignsResponse']['tns:sign']), (sign) => {
          return isValidSign(sign)
        })
        this.setState({
  ...
  focus(location) {
    this.setState({
      mapCentre: location ? location : null
    }, () => {
  ...
  updateMapExtent(bounds) {
    if (bounds) {
      this.setState({cameras: filterCamerasByBoundingBox(this.state.allCameras, bounds)})
    }
  }
  componentDidMount() {
    this.getCameras()
  ...
  render() {
    return (
      <div className={this.props.root}>
        <Grid container spacing={0} direction={"row"} alignContent={"center"} justify={"space-between"} alignItems={"stretch"}>
          ...
          {!this.state.cameras.length ? null :
           this.state.cameras.length <= 10 ?
            _.map(
              this.state.cameras,
              (camera) => {
                return (<Grid key={camera.id} item xs={12} sm={6} md={4} lg={2}>
                  <Camera
                    focus={this.focus}
                    {...camera}
                  />
